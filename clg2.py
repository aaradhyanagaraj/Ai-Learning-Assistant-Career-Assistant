...
# =========================================================
    ...
    resume_file = st.file_uploader("Upload Resume", type=["pdf","docx","txt","png","jpg","jpeg"])
...
After answer → evaluate + improve + next question.
"""
        ...
        answer = st.text_area("Your Answer", key=f"ans_{st.session_state.answer_key}")
        ...
        if col2.button("End Interview"):
            ...
            transcript = "\n".join([m["content"] for m in st.session_state.interview_messages])

            feedback_prompt = f"""
You are ATS + Hiring Manager.
...
"""
            ...
            st.session_state.chat_history.append({
                "id": st.session_state.current_chat_id,
                "messages": st.session_state.interview_messages.copy()
            })
            st.session_state.current_chat_id += 1
